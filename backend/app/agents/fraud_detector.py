...
class FraudDetectorAgent(BaseAnalysisAgent):
    ...
    def _get_fallback_results(self, document_text: str) -> Dict[str, Any]:
        ...
        fraud_keywords = [
            "guaranteed", "risk-free", "no hidden fees", "free trial",
            "cancel anytime", "limited time", "act now", "exclusive offer"
        ]
        ...
        for keyword in fraud_keywords:
            if keyword in text_lower:
                fraud_indicators.append(f"Document contains potentially deceptive language: '{keyword}'")
        ...
        for line in lines:
            ...
            if any(term in line_lower for term in fee_terms):
                if any(suspicious in line_lower for suspicious in ["additional", "extra", "penalty"]):
        ...
        fraud_score = len(fraud_indicators) * 0.5 + len(hidden_fees) * 1.0 + len(suspicious_clauses) * 1.5
