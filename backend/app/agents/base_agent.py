...
class BaseAnalysisAgent(ABC):
    ...
    async def _analyze_with_llm(self, state: DocumentAnalysisState, step_name: str, progress: float) -> DocumentAnalysisState:
        """
        ...
        try:
            ...
            if not self._validate_document_text(state):
                return self._update_state_with_error(
                    state, 
                    f"No document text available for {step_name}", 
                    step_name
                )
            ...
            if llm_response:
                try:
                    ...
                    logger.info(f"{self.agent_name}: LLM analysis completed for file {state['file_id']}")
                    
                except (json.JSONDecodeError, KeyError, ValueError) as e:
                    logger.warning(f"{self.agent_name}: Failed to parse LLM response, using fallback: {e}")
