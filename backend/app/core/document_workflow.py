...
from app.core.langfuse_integration import log_workflow_event, trace_workflow
...
class DocumentAnalysisWorkflow:
    ...
    async def process_document(
        self, file_id: str, file_content: bytes, filename: str, file_type: str
        ...
        try:
            ...
            logger.info(
                f"Document analysis completed for file: {file_id} in {processing_time:.2f}s"
            )
            ...
            log_workflow_event(
                "workflow_failed",
                {"file_id": file_id, "error": str(e), "processing_time": time.time() - start_time},
            )
        ...
        try:
            ...
            if state and state.values:
                return {
                    ...
                    "status": (
                        "processing"
                        if state.values.get("progress_percentage", 0) < 100
                        else "completed"
                    ),
