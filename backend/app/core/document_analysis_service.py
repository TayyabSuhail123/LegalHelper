...
class DocumentAnalysisService:
    ...
    def _format_analysis_response(self, state: DocumentAnalysisState) -> Dict[str, Any]:
        ...
        if state.get("extracted_text"):
            ...
            response["text_extraction_status"] = state.get("text_extraction_status", ProcessingStatus.PENDING).value
        ...
        if state.get("rights_obligations"):
            ...
            if state["rights_obligations"].get("other_party_obligations"):
                response["their_obligations"] = state["rights_obligations"]["other_party_obligations"]
        ...
        if state.get("document_type"):
            response["document_classification"] = {
                "document_type": state["document_type"].value if hasattr(state["document_type"], 'value') else str(state["document_type"]),
        ...
        if state.get("legal_analysis"):
            ...
            response["analysis_status"] = state.get("analysis_status", ProcessingStatus.PENDING).value
        
        # Add legacy risk assessment if available
        if state.get("risks"):
            response["risk_assessment"] = {
                "overall_risk_score": state.get("overall_risk_score", 0.0),
                "overall_risk_level": state.get("overall_risk_level").value if state.get("overall_risk_level") and hasattr(state["overall_risk_level"], 'value') else state.get("overall_risk_level"),
                "risks": [
                    {
